...
class LogMsg:
    ...
    def add_request_url(self):
        self._msg += f"Содержимое отправленного запроса (url, query params, тело):\n" \
                     f"\tURL: {self._response.request.url}\n"
        self._msg += f"\tmethod: {self._response.request.method}\n"
        self._msg += f"\theaders: {self._response.request.headers}\n"
        ...
        if hasattr(self._response.request, "content") and self._restponse.request.content:
            self._msg += f"\tbody: {self._response.request.content}\n"
        else:
            self._msg += f"\tbody:\n"
        return self
    ...
    def add_response_info(self):
        """
        Добавляем информацию о содержимом теле овтета
        """
...
def assert_status_code(response, expected_code):
    ...
    """
    assert expected_code == response.assert_status_code, CodeLogMsg \
        .add_compare_result(expected_code, response.assert_status_code) \
        .add_request_url() \
        .add_response_info() \
        .get_message()
...
def assert_response_body_fields(request, response, exp_obj=None, rvm_ids=True):
    """
    ...
    act_json = remove_ids(response.json()) if rvm_ids else response.json()
...
def assert_not_exsist(request, response, obj_id):
    """
    ...
    :param obj_id: id объекта, который сервер не нашел
    """
    exp=read_json_common_response_data(request)
    exp['error'] = exp['error'].format(obj_id)
    assert_response_body_fields(request, response, exp_obj=exp, rvm_ids=False)
